      ...
      <div class="commonheight" v-if="dataList.length>0">
        ...
        <ul class="wodelist">
          <li v-for="(item,k) in dataList" :key="item.seqNo">
              <div class="clearfix">
              <span class="xh">{{item.seqNo}}</span><span class="wqbh">{{item.rightProtectSn}}</span><span class="sqsj">{{item.createTime|dateFormat}}</span><span class="ywlx">{{item.businessType|wqtypeFormat}}</span><span class="sq">{{item.appealContent}}</span><span class="dqzt">{{item.state|wqstateFormat}}</span><span class="slsj">{{item.acceptTime|dateFormat}}</span>
            <span class="cz">
